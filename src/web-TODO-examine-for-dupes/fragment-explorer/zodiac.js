...
if (typeof window.event != 'undefined') // IE
  ...
  document.onkeypress = function(e)  // FireFox/Others 
	...
	for (var i=0; i<ciphersNew.length; i++) {
		...
		cipher[i] = new Array();
		var j = 0;
		while (j<c.ciphertext.length) {
			cipher[i][cipher[i].length] = c.ciphertext.substring(j,j+17);
			j+=17;
		}
	}
	var cipherLength = new Array(cipher.length);
...
//						if (isPrime1(row*17+col+1)) cell.setAttribute("class","prime");
						if ((row*17+col+1) % 2 == 0) cell.setAttribute("class","even"); 
...
function on(pos) {
	darkenrc(parseInt(pos/17), pos%17);
}
function off(pos) {
	lightenrc(parseInt(pos/17), pos%17);
}
