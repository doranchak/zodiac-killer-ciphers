...
function drawCipher(cipher, width, szx, szy, sx, sy, gapx, gapy, id, color, drawCounter, reps, avg, count) {
	...
	for (var row=0; row<rows; row++) {
		for (var col=0; col<cols; col++) {
			...
			rc2 = "x"+row+"_"+col;
			z--;
			var rot = 10-parseInt(Math.random()*21);
			rot=0;
			var scale=1.1-Math.random()/5;
			scale=1;
			...
			html += "<span id=\"" + rc + "\" class=\"c\" style=\"color: " + color + "; position: absolute; left: " + left + "px; top: " + top + "px; font-size: " + (sizex+10) + "px; padding: 1px 3px 1px 2px; z-index: " + z + "; transform: scale(" + scale+","+scale+") rotate(" + rot + "deg)\">" + cipher[row*width+col] + "</span>";
		}
	}
...
function wiggle(ms) {
	var id = setInterval(frame2, ms);
	function frame() {
		drawCipher(ciphers[0], 17, 23, 23, 100, 30, 2, 2, 'cipher', '#000', false, 0, 0, 0)		
	}
	function frame2() {
		for (var r=0; r<cipherblock.length; r++) {
			for (var c=0; c<cipherblock[r].length; c++) {
  				var rot = 10-parseInt(Math.random()*21);
				var scale=1.1-Math.random()/5;
				
				cipherblock[r][c].style.transform = "rotate(" + rot + "deg) scale(" + scale+","+scale+")";
//				rot = (rot+1) % 360;
//				console.log(str +", " + r + "," + c + ", " + cipherblock[r][c].style);
//				console.log(cipherblock[r][c].style.transform);
			}
		}
	}
}
