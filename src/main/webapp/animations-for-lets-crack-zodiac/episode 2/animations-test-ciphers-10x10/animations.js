...
//z340 extra column width 18
"dHER>pl^VPk|1LTG2d)Np+B(#O%DWY.<*Kf)JBy:cM+UZGW()L#zHJ2Spp7^l8*V3pO++RK2/_9M+ztjd|5FP+&4k/(p8R^FlO-*dCkF>2D(|#5+Kq%;2UcXGV.zL|9(G2Jfj#O+_NYz+@L9Kd<M+b+ZR2FBcyA64K--zlUV+^J+Op7<FBy-OU+R/5tE|DYBpbTMKOF2<clRJ|*5T4M.+&BFRz69Sy#+N|5FBc(;8R+lGFN^f524b.cV4t+++yBX1*:49CE>VUZ5-+2|c.3zBK(Op^.fMqG2LRcT+L16C<+FlWB|)Lp++)WCzWcPOSHT/()pc|FkdW<7tB_YOB*-Cc+>MDHNpkSzZO8A|K;+",
//z340 shift right
"dHER>pl^VPk|1LTG2)Np+B(#O%DWY.<*KfJBy:cM+UZGW()L#zH2Spp7^l8*V3pO++RK/_9M+ztjd|5FP+&4k(p8R^FlO-*dCkF>2D|#5+Kq%;2UcXGV.zL9(G2Jfj#O+_NYz+@LKd<M+b+ZR2FBcyA64--zlUV+^J+Op7<FByOU+R/5tE|DYBpbTMKF2<clRJ|*5T4M.+&BRz69Sy#+N|5FBc(;8+lGFN^f524b.cV4t++yBX1*:49CE>VUZ5-2|c.3zBK(Op^.fMqGLRcT+L16C<+FlWB|)p++)WCzWcPOSHT/()c|FkdW<7tB_YOB*-C+>MDHNpkSzZO8A|K;",
//z340 shift right + p19
...
//z340 sorted by symbol
"AABBBBBBBBBBBBbbbCCCCCccccccccccDDDDdddddEEEffffFFFFFFFFFFGGGGGGHHHH::;;;||||||||||JJJJjjKKKKKKKkkkkkLLLLLLlllllllMMMMMMMNNNNNOOOOOOOOOOPPP&&pppppppppppqqRRRRRRRRSSSSTTTTTttttUUUUUVVVVVV^^^^^^WWWWWWXXYYYYyyyyyZZZZ++++++++++++++++++++++++-----......///<<<<<<>>>>#####%%******@___111222222222334444445555555666zzzzzzzzz((((((()))))77788889999"
)

var z340alphabet = "ABbCcDdEfFGH:;|JjKkLlMNOP&pqRSTtUV^WXYyZ+-./<>#%*@_123456z()789";
...
function qCountBigramsAll(ms) {
	...
	function frame() {
		if (index == bigramQueue.length) {
			...
			var counts = document.getElementById("bigram");
			counts.innerHTML = "BIGRAM: <span style='color: red; font-family: \"Work Sans\"; font-size: 40px'>" + encode(bg) + "</span>";
			counts.style.fontFamily = "Work Sans";
			counts.style.position = "absolute";
			counts.style.top = "370px";
			counts.style.left = "50px";
...
function cipherHighlightBigramsAllTotalButton(which) {
	for (var i=0; i<340; i++) {
	...
	setTimeout(function() {
		cipherHighlightBigramsAllTotal(which)
	}, 2000);
}
function cipherHighlightBigramsAllTotal(which) {
	...
	function frame() {
		...
		var counts = document.getElementById("bigram");
		counts.innerHTML = "BIGRAM: <span style='color: red; font-family: \"Work Sans\"; font-size: 40px'><span id='bg'>" + encode(bigram) + "</span></span>";
		counts.style.fontFamily = "Work Sans";
		counts.style.position = "absolute";
		counts.style.top = "200px";
		counts.style.left = "550px";
		counts.style.fontSize = "40px";
		counts.style.color = "#6aa84f";
...
function reading(ms) {
	...
	var z340 = ciphers[2];
	function frame() {
		// find next occurrence of symbol
		var ch = z340alphabet[alphabetIndex]
		pos = z340.indexOf(ch, pos+1);
		if (pos == -1) {
			ch = z340alphabet[++alphabetIndex];
			pos = z340.indexOf(ch);
		}
...
function drawUnigramChart() {
	...
	var sizex = 23;
	var maxWidth = 0;
	for (var i=0; i<unigramCounts.length; i++) {
		var pair = unigramCounts[i];
		var symbol = pair[0];
		var count = pair[1];
		var width=count*10;
		maxWidth = Math.max(maxWidth, width);
		html += "<span class=\"c\" style=\"color: " + color + "; position: absolute; left: " + left + "px; top: " + top + "px; font-size: " + (sizex+10) + "px; padding: 1px 3px 1px 2px; z-index: " + 0 + "\">" + symbol + "</span>";
		html += "<span class=\"c\" style=\"background-color: #4a86e8; color: white; position: absolute; left: " + (left+30) + "px; top: " + (top-1) + "px; width: " + width + "px; font-size: " + (sizex+10) + "px; padding: 1px 3px 1px 2px; z-index: " + 0 + "\"><span style=\"margin-left: 2px; font-family: arial; font-size: 16pt; \">" + count + "</span></span>";
		top += 30;
		total++;
		if (total % 9 == 0) {
			top = topStart;
...
function drawTestCiphers() {
	var i = 0; var color= "#aaa";
	for (var y=0; y<10; y++) {
		for (var x=0; x<10; x++) {
			if (i>=95) color = "#00f";
			else color = "#aaa";
			drawCipher(testCiphers[i++], 17, 3, 3, 142*x, 147*y, 5, 4, 'cipher', color, false, 0, 0, 0);
		}
	}
}
