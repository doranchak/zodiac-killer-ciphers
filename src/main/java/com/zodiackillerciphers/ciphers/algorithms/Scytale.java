...
public class Scytale {
	...
	/** encode input as scytale.  instead of reading the columns out sequentially, read them in the given order. */ 
	public static String encode(String input, int period, int[] order, boolean reverse) {
		return Periods.rewrite3(input, period, order, reverse);
	}
	...
	public static void test() {
		...
		String inp = "IHOPEYOUAREHAVINGLOTSOFFUNINTRYINGTOCATCHMETHATWASNTMEONTHETVSHOWWHICHBRINGORPAPOINTABOUTMEIUMNOTAFRAIFOFTHEGASCHAMBERBECUASEITWILLSENDMETOPAYUNLCEALLTHE";
		String en = encode(inp, 9);
		String de = decode(en, 9);
		...
		System.out.println("for " + inp);
		for (int i=2; i<171; i++) {
			System.out.println(i + ": " + encode(inp, i));
		}
		System.out.println("==== non sequential column order ====");
		System.out.println(encode(inp, 4, new int[] {2,3,0,1}, false));
