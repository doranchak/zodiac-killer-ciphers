...
public class MyNameIs {
	...
	public static boolean fitSub(StringBuffer sb) { return fitSub(sb, false); }
	
	/** if circledEightsAreVowels is true, treat circled eights as vowel wildcards (http://www.zodiackillersite.com/viewtopic.php?p=69081#p69081) */
	public static boolean fitSub(StringBuffer sb, boolean circledEightsAreVowels) {
		if (sb.charAt(0) != sb.charAt(11))
			return false;
		if (sb.charAt(7) != sb.charAt(12))
			return false;
		if (sb.charAt(2) != sb.charAt(10))
			return false;
		if (circledEightsAreVowels) {
			if (!(vowels.contains(sb.charAt(4)) && vowels.contains(sb.charAt(6)) && vowels.contains(sb.charAt(8)))) {
				return false;
			}
		} else if (!(sb.charAt(4) == sb.charAt(6) && sb.charAt(6) == sb.charAt(8)))
			return false;
		return true;
	}
