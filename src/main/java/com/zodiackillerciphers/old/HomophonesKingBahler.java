...
public class HomophonesKingBahler {
	...
	/** score for strings */
	List<Object[]> scores; /* 
		0: (String) repeated string
		1: (Integer) count of strings
		2: (Integer) count of appearances
		3: (Float) ratio of appearances to total strings
		4: (Float) F (string length * appearance count * ratio * unseenRatio)
		5: (String) full sequences
	*/
	...
	/** go through strings.  for any that have more than 2 repeating cycle, add its contribution to an overall score, 
	 * exponential in L (the length of the cycle).  reward cycles that have more repeats.
	 * 
	 *  this.scores should already be computed before calling this.
	 */
	long scoreAppearances() {
		return scoreAppearances(false);
	}
	long scoreAppearances(boolean dump) {
		long sum = 0;
		
		// map L to sum of appearances
		Map<Integer, Integer> map = new HashMap<Integer, Integer>();
		// map L to distinct set of symbols involved 
		Map<Integer, Set<Character>> mapSymbols = new HashMap<Integer, Set<Character>>();
		
		for (Object[] o : scores) {
			int appearances = (int)o[2]; 
			if (appearances < 3) continue; // must have at least 3 appearances
			String cycle = (String) o[0];
			if (dump) {
				String line = cycle.length() + "	" + o[0];
				line += "	" + appearances;
				System.out.println(line);
			}
			Integer val = map.get(cycle.length());
			if (val == null) val = 0;
			val = val + (int) Math.pow(appearances-2, 3); // reward higher appearance counts by cubing them 
			map.put(cycle.length(), val);
			
			Set<Character> set = mapSymbols.get(cycle.length());
			if (set == null) {
				set = new HashSet<Character>();
				mapSymbols.put(cycle.length(), set);
			}
		...
		if (dump) System.out.println("map " + map);
		if (dump) System.out.println("mapSymbols " + mapSymbols);
		for (Integer L : map.keySet()) {
			if (dump) System.out.println("sum for L" + L + ": " + map.get(L));
			sum += Math.pow(map.get(L), L-1);
		}
		
		return sum;
	}
	public static long scoreAppearances(String c) {
		HomophonesKingBahler h = new HomophonesKingBahler(c, 0);
		h.translateCipher();
		h.findStrings();
		h.reduceStrings();
		h.scoreStrings(WHICH_SPECIAL_COUNT);
		return h.scoreAppearances();
	}
	
	public static void shuffleAppearances(String cipher, int shuffles) {
		System.out.println(cipher);
		StatsWrapper stats = new StatsWrapper();
		stats.actual = scoreAppearances(cipher);
		for (int i=0; i<shuffles; i++) {
			cipher = CipherTransformations.shuffle(cipher);
			stats.addValue(scoreAppearances(cipher));
		}
		stats.output();
		
	}
	
	/** use similar measurement as HomophonesNew.perfectCycleScoreFor, but only care about perfect unbroken
	 * cycle at beginning of full sequence
	 * 
	 * if bestPerSymbol true, then keep only the best scores per symbol, instead of counting
	 * every cycle 
	 */
	public static double scorePerfectCycles(String cipher, int L, boolean dump, boolean bestPerSymbol) {
		int minReps = 3;
		HomophonesKingBahler h = new HomophonesKingBahler(cipher, 0);
		h.translateCipher();
		h.findStrings();
		h.reduceStrings();
		h.scoreStrings(WHICH_SPECIAL_COUNT);
		// process the sequences, arrange by number of perfect cycles in a row
		Map<Integer, Double> byRun = new HashMap<Integer, Double>();
		Map<Character, Double> bestMap = new HashMap<Character, Double>(); 
		Map<Character, Integer> bestMapRuns = new HashMap<Character, Integer>(); 
		Set<String> seen = new HashSet<String>(); 
		for (Object[] o : h.scores) { /* 
			0: (String) repeated string
			1: (Integer) count of strings
			2: (Integer) count of appearances
			3: (Float) ratio of appearances to total strings
			...
			5: (String) full sequences
		*/
			int len = ((String)o[0]).length();
			if (len != L) continue;
			String full = (String) o[5];
			if (seen.contains(full)) continue; // don't overcount dupes
			seen.add(full);
			String first = full.substring(0, len);
			int run = 1;
//			System.out.println(full + ", " + first);
			for (int i=len; (i+len-1)<full.length(); i+=len) {
				String sub = full.substring(i,i+len);
				if (!first.equals(sub)) break;
				run++;
			}
			if (run < minReps) continue;
			Double val = byRun.get(run);
			if (val == null) val = 0d;
			double percent = run*len;
			percent /= full.length(); // percentage of full sequence covered by perfect cycles
			double pow = Math.pow(percent, 3);
			val += pow; // exaggerate imperfections
//			val += (run-minReps+1);
			byRun.put(run, val);
			if (dump) {
				System.out.println(run + " " + first + " " + percent + " " + val + " " + full);
			}
			if (bestPerSymbol) {
				for (int i=0; i<first.length(); i++) {
					char c = first.charAt(i);
					Double best = bestMap.get(c);
					if (best == null) best = 0d;
					if (pow > best) {
						bestMap.put(c, pow);
						bestMapRuns.put(c, run);
					}
				}
			}
		}
		
		double runLengthFactor = 2.0;
		double score = 0d;
		
		if (bestPerSymbol) {
			for (Character c : bestMap.keySet()) {
				double val = bestMap.get(c);
				int run = bestMapRuns.get(c);
				int exp = run-minReps+1;
				double add = val * Math.pow(runLengthFactor, exp); // exponential rewards for longer run lengths
				score += add;
			}
			return score;
		}
	...
	/** composite score of perfect cycles */
	public static double scorePerfectCycles(String cipher) {
		return scorePerfectCycles(cipher, false, false);
	}
	public static double scorePerfectCycles(String cipher, boolean dump, boolean bestPerSymbol) {
		double score = 0d;
		int L = 2;
		while (true) {
			int exp = L-1;
			double pcs = scorePerfectCycles(cipher, L, dump, bestPerSymbol);
			if (pcs == 0) break;
			score += Math.pow(pcs, exp);
			L++;
		}
		return score;
	}
	...
	public static void testFindStrings2(String c, String msg) {
		...
		h.scoreStringsBestBySymbol(true);
		h.dumpScores();
		System.out.println("Appearances score: " + h.scoreAppearances(true));
		System.out.println("Combinations tested: " + h.combinations);

		System.out.println(h.bestBySymbol() + "," + msg);
		System.out.println(h.strings);
	}
...
//			List<Object[]> scores; // [repeated string, count of strings, count of appearances, ratio of appearances to total strings, F (string length * appearance count)] 
	...
	public static void testScoreAppearances(String cipher, boolean dump) {
		boolean bestPerSymbol = false;
		System.out.println("Cipher: " + cipher);
		for (int L=2; L<10; L++) {
			System.out.println(L + ": " + scorePerfectCycles(cipher, L, dump, bestPerSymbol));
			System.out.println(L + ": " + scorePerfectCycles(cipher, L, dump, !bestPerSymbol));
		}
		System.out.println("Composite pcs: " + scorePerfectCycles(cipher, false, bestPerSymbol));
		System.out.println("Composite pcs: " + scorePerfectCycles(cipher, false, !bestPerSymbol));
//		testFindStrings2(cipher, "cipher");
		for (int L=2; L<4; L++) System.out.println("L"+L+": " + HomophonesNew.perfectCycleScoreFor(L, cipher, 3, false));
		System.out.println("jarlve homscore: " + JarlveMeasurements.homScore(cipher));
	}
	public static void testScoreAppearances() {
//		String cipher = "9%P/Z/UB%kOR=pX=BWV+eGYF69HP@K!qYeMJY^UIk7qTtNQYD5)S(/9#BPORAU%fRlqEk^LMZJdr\\pFHVWe8Y@+qGD9KI)6qX85zS(Ld/P#B@XqEHMU^RRkcZKqpI)Wq!85LMr9#BPDR+j=6\\N(eEUHkFZcpOVWI5+tL)l^R6HI9DR_TYr\\de/@XJQAP5M8RUt%L)NVEKH=GrI!Jk598LMlNA)Z(PzUpkA9#BVW\\+VTtOP^=SrlfUe67DzG%%IMNk)ScE/9%%ZfAP#BVpeXqWq_F#8c+@9A9BH!FBX9zXADd\\7L!=q_ed##6e5PORXQF%GcZ@JTtq_8JI+rBPQW6"; // deleted rows: 6, 23, 19, 18
		...
		for (String cipher : ciphers) 
			testScoreAppearances(cipher, false);
	}
	public static void main(String[] args) {
//		testScoreAppearances();
		testFindStrings2("HER>pl^VPk|1LTG2dNp+B(#O%DWY.<*Kf)By:cM+UZGW()L#zHJSpp7^l8*V3pO++RK2_9M+ztjd|5FP+&4k/p8R^FlO-*dCkF>2D(#5+Kq%;2UcXGV.zL|(G2Jfj#O+_NYz+@L9d<M+b+ZR2FBcyA64K-zlUV+^J+Op7<FBy-U+R/5tE|DYBpbTMKO2<clRJ|*5T4M.+&BFz69Sy#+N|5FBc(;8RyBX1*:49CE>VUZ5-+|c.3zBK(Op^.fMqG2|FkdW<7tB_YOB*-Cc","");
		//testTranslateCipher(Ciphers.cipher[1]);
		//testFindStrings2(Ciphers.cipher[1].cipher,"408");
//		cipher = CipherTransformations.shuffle(cipher);
//		System.out.println(scoreAppearances(Ciphers.Z408));
//		System.out.println(scoreAppearances(Ciphers.Z340));
//		shuffleAppearances(Ciphers.Z408, 100000);
