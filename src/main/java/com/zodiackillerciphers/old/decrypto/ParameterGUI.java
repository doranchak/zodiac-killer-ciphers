    ...
    **/
	...
	JSlider getSlider()
	{
	    if (slider == null)
		{
		    ...
		    slider.addChangeListener(new ChangeListener() {
			    public void stateChanged(ChangeEvent e) {
				setDoubleValue(((double)slider.getValue())/SLIDER_CLICKS*(max-min)+min);
			    }});
		}
	    return slider;
	}
	...
	for (ParameterListener pl : listeners)
	    pl.parameterChanged(ParameterGUI.this, name);
    }
    ...
    {
	DoubleValue val = new DoubleValue(name, desc, -Double.MAX_VALUE, Double.MAX_VALUE, value);
	parammap.put(name, val);
