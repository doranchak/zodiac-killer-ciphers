...
public class EvenOddAndFactors {
	...
	/** get the number of repeating ngrams for evens, and the number for odds.  then return the difference. */
	public static int differenceEvenOdds(String cipher) {
	...
	public static void testShuffles(String cipher, int shuffles) {
		StatsWrapper stats = new StatsWrapper();
		stats.actual = differenceEvenOdds(cipher);
		stats.histogram = true;
		
		for (int i=0; i<shuffles; i++) {
			cipher = CipherTransformations.shuffle(cipher);
			int diff = differenceEvenOdds(cipher);
			stats.addValue(diff);
	...
	public static void main(String[] args) {
		...
		testShuffles(Ciphers.Z340, 1000000);
//		System.out.println(differenceEvenOdds(Ciphers.Z340));
