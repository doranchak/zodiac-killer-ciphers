...
public class HomophonicThreadOriginal extends Thread {
	...
	public void run() {
		NamestufferSolution hom = new NamestufferSolution(cipher, plaintext, null);
		hom.initialize();
		boolean go = true;
		...
		while (temperature >= 1) {
			int fails = 0;
			while (go) {
				hom = (NamestufferSolution) SimulatedAnnealing.run(temperature, 100000, hom, threadNum);
...
//		}
	...
	public static void testStats() {
		String[] plaintexts = new String[] { Ciphers.Z408_SOLUTION.toUpperCase(),
				...
				"",
				"" };
		for (String plaintext : plaintexts) NamestufferSolution.dumpStats(plaintext);
	}
}
