...
public class Spiral extends TransformationBase {
	...
	public Spiral(List<StringBuffer> input, Integer reverse) {
		this.input = input;
		setParameterValue("reverse", reverse);
	}
	public Spiral(List<StringBuffer> input, Float reverse) {
		this.input = input;
		setParameterValue("reverse", reverse);
	}
	...
	public static String transform(String cipher, int width, boolean reverse) {
		List<StringBuffer> list = TransformationBase.toList(
				cipher, width);
		Transformation t = new Spiral(list, reverse ? 1 : 0);
		t.execute(false);
