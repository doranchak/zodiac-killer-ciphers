...
public class Operations extends FloatVectorIndividual implements FrontMember {
	...
	public static String CIPHER_STRING = "+E7'D*!$3*.)HSF1$M&^*%Y6[ZU=($VQ,*$R+>)*'/KG$(*8V$BITC#X3>E14#]+H\\LX-3*=$)*@II^1HQ$[>!YP^<R\\U:T,<?$IO;#QK*5A\"$9I0&CE*^.+0D]M/JV$!K'*QR46)-4Z\"MI$8H1?9'?E&7^A?(!-B)\\V?Z+55A*Q_M9KX+G_/$2-4%E]R!;SFL)_<*8\"1I3&#N=#HL';Y$O0Z*F@[S!X.V\"':$*-^V\\:DK3UF,*E!8AB\"TQ$7K>[4=*[\"^\\H#$XME*T3C49+[]PG>-;%5#X*\\&N)T/RE1@?N3>2S\\IQ#]P$P'[JU*;O*:1@@K$*#$U:?RW2,Y*9E";
	...
	static {
		referenceHomophones = HomophonesNew.beansFor(2, CIPHER_STRING, false);
	...
	public double[] measure(String cipher, int numoperations, String plaintext) {
		try {
			...
			lastObjectives[1] = JarlveMeasurements.homScore(cipher);
	...
	public static void testMeasureFragments() {
		String cipher = Ciphers.cipher[1].cipher;
		System.out.println(Operations.measureFragments(cipher, 25));
		for (int n=1; n<=170; n++) {
			String re = Periods.rewrite3(cipher, n);
			System.out.println(Operations.measureFragments(re, 25) + " " + n);
		}
