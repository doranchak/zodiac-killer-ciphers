...
public class Anagrams {
	...
	/** look for dictionary words that fit in the given string.  for each word found, remove it from
	 * string and keep finding words in what remains (to a limited extent). */
	public static void anagramsFor(String str, int minLength, int maxPerLevel) {
		WordFrequencies.init();
		anagramsFor(str, minLength, maxPerLevel, 0);
	}
	public static void anagramsFor(String str, int minLength, int maxPerLevel, int level) {
		...
		for (String word : WordFrequencies.map.keySet()) {
			...
			if (anagram(word.toUpperCase(), str)) {
				...
				anagramsFor(newStr, minLength, maxPerLevel, level+1);
				found++;
				if (level > 0 && found > maxPerLevel) break;
			}
		}
