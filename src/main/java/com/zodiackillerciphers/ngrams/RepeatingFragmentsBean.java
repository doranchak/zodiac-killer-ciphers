...
//				System.out.println(ch + " freq = " + cipherSymbolFrequencies.get(ch));
			}
//			System.out.println("product: " + result);
//			System.out.println("repeats: " + counts.get(key).size());
	...
	public void makeBeans(boolean excludeNonSkipgrams, int minNonWildcards) {
		for (String key : counts.keySet()) {
			if (!excludeNonSkipgrams || key.contains("?")) {
				int nonWilds = 0;
				for (int i=0; i<key.length(); i++) {
					if (key.charAt(i) != '?') nonWilds++;
					if (nonWilds > minNonWildcards) {
						break;
					}
				}
				if (nonWilds < minNonWildcards) continue;
	...
	/** count up how many positions are covered by non-wildcard portions of repeating fragments.
	 * return ratio of count to cipher size.
	 */
	public float coverage() {
		Set<Integer> uniqPos = new HashSet<Integer>();
		for (RepeatingFragmentsBeanEntry bean : beans) {
			for (int pos : bean.positionsInt) {
				for (int i=0; i<bean.key.length(); i++) {
					if (bean.key.charAt(i) == '?') continue;
					uniqPos.add(pos+i);
				}
			}
		}
		float result = uniqPos.size();
		result /= cipherLength;
		return result;
	}
