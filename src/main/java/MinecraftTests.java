...
public class MinecraftTests {
	...
	/** exploring layouts for 2x2 tree farms within a width of 29 */
	public static float twoByTwoTrees(int N, int W) {
		float E = 0;
		E = 29-2*N-W*(N-1);
		E /= 2.0;
		return E;
	...
	public static void woolChests(int dubs) {
		for (int a=1; a<10; a++) {
			for (int b=1; b<5; b++) {
				int stacks = a*16+b*16; 
				if (stacks < dubs*54) {
					System.out.println(stacks + " " + a + " " + b);
				}
			}
		}
	}
...
//		for (int N=1; N<15; N++) {
//			for (int W = 2; W<=10; W++) {
//				float E = twoByTwoTrees(N, W);
//				if (E < 0) continue;
//				if (E == (int) E) {
//					System.out.println(N + " " + W + " " + twoByTwoTrees(N, W));
//				}
//			}
//		}
		woolChests(3);
