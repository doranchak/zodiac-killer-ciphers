...
//		alert(c3); alert(c4);
	...
	function getImg(s) {
		...
		for (var i=0; i<s.length; i++) {
			result += "<img src=\"alphabet2/" + getName(s.substring(i,i+1)) + ".jpg\">";
		}
		return result;
	}
	/** get image corresponding to given string of cipher letters */
...
//		e.className += "_darker";
		...
		if (imgsize) {
			e.children[0].height=imgsize;
			e.children[0].width=imgsize;
		}
	}
	...
	function lightenrc(row, col) {
		var e = document.getElementById(row+"_"+col);
		if (e == null) return;
		e.innerHTML = getImg(cipher[which][row].charAt(col));
		if (imgsize) {
			e.children[0].height=imgsize;
			e.children[0].width=imgsize;
		}
