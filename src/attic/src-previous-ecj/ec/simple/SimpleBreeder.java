...
public class SimpleBreeder extends Breeder
    ...
    public void setup(final EvolutionState state, final Parameter base) 
        ...
        //reevaluateElites = new boolean[50];
    ...
    public Population breedPopulation(EvolutionState state) 
        ...
        if (Utils.DEBUG) {
        	for (Individual ind : state.population.subpops[0].individuals) {
        		if (ind instanceof CandidateKey) {
        			...
        			Utils.debug("BEFORE BREED POP, KEY DUMP " + Utils.info(key) + " ARCHIVE? " + key.archive);
        		}
        	}
        }
        ...
        if (Utils.DEBUG) {
        	for (Individual ind : newpop.subpops[0].individuals) {
        		if (ind instanceof CandidateKey) {
        			CandidateKey key = (CandidateKey) ind;
        			Utils.debug("AFTER BREED POP, KEY DUMP " + Utils.info(key) + " ARCHIVE? " + key.archive);
        		}
        	}
        }
        return newpop;
    ...
    protected void breedPopChunk(Population newpop, EvolutionState state,
    	...
    	for (Individual ind : newpop.subpops[0].individuals) {
    		...
    		if (ck.isBroken()) {
    			try {
    				throw new RuntimeException("bad ind!");
    			} catch (Exception e) {
    				e.printStackTrace();
